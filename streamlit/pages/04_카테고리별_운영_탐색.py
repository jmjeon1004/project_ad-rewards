...
from src.components import (render_kpi_card, render_context_message, render_alert_card,
                            render_ad_list_table_with_scores, render_media_summary_table,
                            render_ml_insight, render_chat_section)
from src.charts import make_bubble_chart, make_pareto_chart
from src.filters import render_page_header, render_filters, render_applied_chips
from src.rules import generate_alerts, classify_quadrant
...
# ── 페이지 헤더 ──
render_page_header("카테고리별 운영 탐색")
...
# ── 캐싱용 핑거프린트 ──
_filter_fp = f"p4|{period}|{sorted(selected_groups)}|{h_start}-{h_end}|{c_start}-{c_end}"
_group_fp = f"p4g|{sorted(selected_groups)}|{h_start}-{h_end}"

# ── 데이터 필터링 (session_state 캐싱) ──
def _apply_group_filter_p4(df, groups=selected_groups, hs=h_start, he=h_end):
    gf = df[df[group_col].isin(groups)]
    return filter_by_hour(gf, hs, he)

group_filtered = _cached_filter("p4_group_filtered", _group_fp, base, _apply_group_filter_p4)

def _apply_period_filter_p4(df, p=period, t=today, cs=c_start, ce=c_end):
    return filter_by_period(df, p, t, cs, ce)

filtered = _cached_filter("p4_filtered", _filter_fp, group_filtered, _apply_period_filter_p4)
...
render_applied_chips(
    page_key="p4", filtered_count=len(filtered),
    group_items=group_items, group_label="카테고리",
)
...
# KPI 카드 4종
# ════════════════════════════════════════════════════
kpis = _cached_compute("p4_kpis", _filter_fp, calc_all_kpis, filtered)
full_daily_kpis = _cached_compute("p4_daily_kpis", _group_fp, calc_daily_kpis, group_filtered)
...
change_cvr, change_turn, change_mr = None, None, None
if prev_start is not None:
    _prev_fp = f"p4_prev|{sorted(selected_groups)}|{h_start}-{h_end}|{prev_start}|{prev_end}"
    ...
    prev_kpis = _cached_compute("p4_prev_kpis", _prev_fp, _compute_prev_kpis)
    if prev_kpis:
...
">
    ...
    <span style="margin-left:12px; color:#888;">모든 지표는 ▲ 가 좋음</span>
</div>
...
# 카테고리별 종합 평가 버블 차트
# ════════════════════════════════════════════════════
group_kpis = _cached_compute("p4_group_kpis", _filter_fp, calc_group_kpis, filtered, group_col)
...
with st.container(border=True):
    ...
    with _main_left:
        ...
        x_metric = st.radio(
            "X축", ["cvr", "cpa"],
            format_func=lambda x: {"cvr": "CVR", "cpa": "CPA"}[x],
            horizontal=True, key="p4_xaxis",
        )

        _bubble_fp = f"p4_bubble|{_filter_fp}|{x_metric}"
        _bubble_cached = _cached_compute("p4_bubble_result", _bubble_fp,
                                          lambda: make_bubble_chart(group_kpis, group_col,
                                                                     x_metric=x_metric, y_metric="margin_rate",
                                                                     size_metric="clk", show_breakeven=False))
    ...
    # 닫기 버튼 세션 관리
    if _clicked_name and _clicked_name != st.session_state.get("p4_last_click_name"):
        st.session_state.p4_hide_adlist = False
        st.session_state.p4_last_click_name = _clicked_name
    if not _clicked_name:
        st.session_state.p4_last_click_name = None

    _show_panel = _clicked_name and not st.session_state.get("p4_hide_adlist", False)
    ...
    with _main_right:
        if _show_panel:
            ...
            if overlap_list and len(overlap_list) > 1:
                ...
                with _tg:
                    _selected_name = st.radio(
                        "겹치는 카테고리 선택", overlap_list,
                        horizontal=True, key="p4_overlap_toggle",
                    )
                with _xc:
                    if st.button("✕", key="p4_close_adlist"):
                        st.session_state.p4_hide_adlist = True
                        st.rerun()
            else:
            ...
            x_label = {"cvr": "CVR", "cpa": "CPA"}[x_metric]
            sort_order = "오름차순" if x_metric == "cpa" else "내림차순"
            st.caption(f"{x_label} {sort_order} · 클릭 30건 이상")
            ...
            sort_col = x_metric
            ascending = (x_metric == "cpa")
            render_ad_list_table_with_scores(ad_agg, model_scores, sort_col=sort_col, ascending=ascending,
                                              hide_cols=("매체",))
        else:
            st.markdown(
    ...
    # 운영 알림 4종 카드
    # ════════════════════════════════════════════════════
    quadrants = classify_quadrant(group_kpis, x_metric=x_metric, y_metric="margin_rate")

    _x_label_map = {"cvr": "CVR", "cpa": "CPA"}
    _x_label = _x_label_map[x_metric]

    c1, c2, c3, c4 = st.columns(4)
    with c1:
        best_names = [r.iloc[0] for r in quadrants["best"]]
        render_alert_card("★", f"최우수 ({len(best_names)}건)",
                          best_names if best_names else ["없음"], "green",
                          "광고비 확대 검토")
    with c2:
        exp_names = [r.iloc[0] for r in quadrants["expensive"]]
        render_alert_card("↺", f"소재 개선 ({len(exp_names)}건)",
                          exp_names if exp_names else ["없음"], "blue",
                          f"마진↑ {_x_label}↓ — 소재/타겟 개선")
    with c3:
        low_names = [r.iloc[0] for r in quadrants["low_eff"]]
        render_alert_card("⚠", f"단가 점검 ({len(low_names)}건)",
                          low_names if low_names else ["없음"], "yellow",
                          f"{_x_label}↑ 마진↓ — 단가 효율 점검")
    with c4:
        loss_names = [r.iloc[0] for r in quadrants["loss"]]
        render_alert_card("⊘", f"비효율 ({len(loss_names)}건)",
                          loss_names if loss_names else ["없음"], "red",
                          "즉시 축소 검토")
...
# ════════════════════════════════════════════════════
with st.container(border=True):
    st.markdown("### 카테고리별 완료수 비중(누적)")
    _pareto_fp = f"p4_pareto|{_filter_fp}"
    _pareto_cached = _cached_compute("p4_pareto_result", _pareto_fp,
                                      lambda: make_pareto_chart(group_kpis, group_col))
    pareto_fig, pareto_insight = _pareto_cached
    st.plotly_chart(pareto_fig, use_container_width=True, key="p4_pareto")
    if pareto_insight:
        st.markdown(
            f'<div style="background:{COLORS["legend_bg"]};padding:10px 20px;border-radius:8px;'
            f'font-size:13px;color:#444;">📌 {pareto_insight}</div>',
            unsafe_allow_html=True,
        )
...
render_media_summary_table(
    ...
    group_label="카테고리", widget_key_prefix="p4_summary",
)
...
# ════════════════════════════════════════════════════
with st.container(border=True):
    _p4_thr_key = st.session_state.get("ml_m2_threshold_p4", "best_f1")
    _p4_fp = f"p4|{period}|{sorted(selected_groups)}|{h_start}-{h_end}|{c_start}-{c_end}|{_p4_thr_key}"
    _p4_insight = get_cached_ml_insight_data(
        page_key="p4",
        fingerprint=_p4_fp,
        filtered=filtered,
        ...
        page_name="카테고리별 운영 탐색",
        threshold_key=_p4_thr_key,
        opp_top_n=1,
        risk_top_n=1,
    )
    render_ml_insight(_p4_insight, page_key="p4")
...
_p4_ad_summary = _cached_compute("p4_ad_summary", _filter_fp, _build_chat_summary)

render_chat_section(
    page_key="p4",
    ad_summary=_p4_ad_summary,
    kpis=kpis,
    page_name="카테고리별 운영 탐색",
    filters_desc=f"기간: {period} / 카테고리: {', '.join(selected_groups)} / 시간: {h_start}~{h_end}시",
)
