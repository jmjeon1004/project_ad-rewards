...
- **일일 리포트 자동 생성**: P1 진입 시 질의 없이 오늘 vs 전일 KPI를 근거로 리포트를 자동 생성해 상단 카드에 노출. 채팅에서 "오늘자 리포트 써줘" 등으로도 요청 가능
- **과거 리포트 누적**: 생성된 일일 리포트는 지식베이스에 자동 저장(임베딩)되어, 이후 "과거 리포트" 질의 시 RAG로 재검색됨 (`scripts/backfill_daily_reports.py`로 과거 기간 일괄 백필 가능)
