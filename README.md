...
> **13조 최종 프로젝트** | Streamlit 기반 광고 성과 분석 + ML 품질 등급 + 조기부진 예측 + AI Agent 통합 운영 의사결정 지원 도구
...
| 페이지 | 핵심 기능 |
|--------|-----------|
| **P1 전체 오버뷰** | 페이지 진입 시 질의 없이 자동 노출되는 "오늘의 AI 해석 리포트" 카드<br>전체 기간 모드: KPI 카드 + 시간대×요일 CVR 히트맵 + Top 10 광고 리스트<br>당일 vs 평시 모드: CVR 추이 라인차트(시간 클릭 → 이상 광고) + 차원별 Top 3 카드 |
| **P2 유형별 탐색** | 기간/유형/시간대 필터 + 이동 조치 필요 광고 패널(즉시조치/우선검토/매체확장/승급추진) + 유형별 광고 리스트 |
| **P3 매체별 탐색** | CPA×마진율×완료수 버블 차트(클릭 → 매체 광고 리스트) + 매체 인사이트 카드 3종(최우수/비효율/소재개선) |
| **P4 카테고리별 탐색** | CVR or CPA 선택형 버블 차트 + 운영 알림 4종(★최우수/↺소재개선/⚠단가점검/⊘비효율) + 파레토 차트 |
| **P5 광고 상세** | M1 등급 or M2 부진확률 스코어 카드 + KPI 그리드 + 배지별 액션(매체확장/승급추진/즉시조치/우선검토) + 판정 근거 설명 |
...
└── streamlit/                     # 대시보드 앱
    ...
    │   ├── rag_artifacts/         # RAG 인덱스 (index.parquet, 3072차원 임베딩)
    │   └── preprocessing_pipeline.joblib
    └── data/                      # 대시보드용 Parquet 파일 12개
        └── rag_docs/              # RAG 지식베이스 원본 문서 (PRD, 운영 룰 등)
            └── reports/           # 자동 생성/백필된 일일 리포트 (과거 리포트 검색 대상)
```
...
- **RAG 지식베이스**: PRD, 운영 룰, ML 등급 정의, 과거 일일 리포트 등 정책성·이력성 질문에 활용
- **일일 리포트 자동 생성**: P1 진입 시 질의 없이 오늘 vs 전일 KPI를 근거로 리포트를 자동 생성해 상단 카드에 노출. 채팅에서 "오늘자 리포트 써줘" 등으로도 요청 가능하며, 생성 즉시 RAG 지식베이스에 임베딩되어 이후 "과거 리포트" 질의에 재검색됨 (`scripts/backfill_daily_reports.py`로 과거 기간 일괄 백필 가능)
